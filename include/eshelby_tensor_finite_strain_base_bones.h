...
public:
    ...
    constexpr inline auto num_evaluate(){
        auto matrix_material = dynamic_cast<finite_strain_solid_material_base<_T, _Dim, _Container>*>(_base_material);
        //tmech::tensor<_T,_Dim,4> F = matrix_material->mixed_tangent_tensor();
        ...
        for(std::size_t i{0}; i<size; ++i){
            tmech::tensor<_T,_Dim,1> x{std::sin(theta[i])*std::cos(phi[i])/a1, std::sin(theta[i])*std::sin(phi[i])/a2, std::cos(theta[i])/a3};
            const tmech::tensor<_T,_Dim,2> invA{tmech::inv(x*tmech::transl(F)*x)};
            const auto w = 4*M_PI*weights[i];
            integrate(invA,x,w);
        }
